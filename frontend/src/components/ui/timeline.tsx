  ...
  const handleDragMove = (event: any) => {
    ...
    if (over) {
      ...
      if (slot) {
        ...
        // Validate with host
        const valid = onValidateDrop
          ? onValidateDrop(active.id, newTime, over.id)
          : true;
        setIsValid(valid);
      }
    }
          ...
          {localActiveSlot &&
            (() => {
              const activeSlot = slots.find(
                (s: TimelineSlotData) => s.id === localActiveSlot,
              );
...
export function TimelineHeader({
  ...
  return (
    <div
      ...
      className={cn("sticky top-0 z-10 bg-background border-b", className)}
    >
      <div className="flex h-12">
        <div
          ref={columnRef}
          data-slot="timeline-header-column"
          className="sticky left-0 w-(--timeline-column-width) bg-background border-r flex items-center px-4 font-semibold text-sm z-1"
        >
          {columnLabel}
        </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="sticky left-0 w-(--timeline-column-width) bg-inherit border-r flex items-center px-0 z-5"
      >
...
export function TimelineSlotLabel({
  ...
  return (
    <Comp
      ...
      className={cn("font-medium truncate text-xs", className)}
      {...props}
    />
  );
}
...
function TimelineMouseIndicator({
  mouseX,
  time,
}: {
  mouseX: number;
...
export function TimelineCurrentTime({
  ...
  return (
    <div
      ...
      className={cn(
        "absolute top-0 bottom-0 w-0.5 bg-secondary pointer-events-none z-15",
        className,
      )}
