...
const HudCorners = ({
  ...
  const colorClasses =
    color === "primary" ? "border-primary/80" : "border-muted-foreground/30";

  return (
    <div className={cn("relative", className)}>
      <div
        className={`${cornerClasses} ${colorClasses} top-0 left-0 border-t border-l`}
      ></div>
      <div
        className={`${cornerClasses} ${colorClasses} top-0 right-0 border-t border-r`}
      ></div>
      <div
        className={`${cornerClasses} ${colorClasses} bottom-0 left-0 border-b border-l`}
      ></div>
      <div
        className={`${cornerClasses} ${colorClasses} bottom-0 right-0 border-b border-r`}
      ></div>
      {children}
    </div>
  );
