...
async function horizonAccountExists(address: string): Promise<boolean> {
  ...
  if (!res.ok)
    throw new Error(`Horizon error ${res.status} checking account existence`);
  return true;
}
...
export async function getSimulationSourceAddress(
  avoidAddresses: string[] = [],
): Promise<string> {
...
export async function getFundedSimulationSourceAddress(
  avoidAddresses: string[] = [],
): Promise<string> {
