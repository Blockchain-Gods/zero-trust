...
class DevWalletService {
  ...
  static isDevModeAvailable(): boolean {
    ...
    return !!(
      process.env.NEXT_PUBLIC_DEV_PLAYER1_SECRET &&
      process.env.NEXT_PUBLIC_DEV_PLAYER2_SECRET
    );
  }
  ...
  async initPlayer(playerNumber: 1 | 2): Promise<void> {
    try {
      ...
      if (!secretEnvVar || secretEnvVar === "NOT_AVAILABLE") {
        throw new Error(
          `Player ${playerNumber} secret key not available. Run "bun run setup" first.`,
        );
      }
      ...
      console.log(
        `Dev wallet initialized for Player ${playerNumber}: ${keypair.publicKey()}`,
      );
    } catch (error) {
  ...
  getSigner(): ContractSigner {
    ...
    const toWalletError = (message: string): WalletError => ({
      message,
      code: -1,
    });
